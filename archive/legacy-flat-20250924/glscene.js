/* Nasjicad — glscene.js (GL-PLAN.md phase 3: completeness)
 ...
 *
 * - PRESENTATION (the 4.6ms/frame webgl→2d drawImage sync is gone from
 ...
 *   run carries a z group (bumped at line↔fill transitions), opaque geometry
...
(() => {
  ...
  const MODE_KEY = 'nasjicad.glscene';   /* 'auto' | 'on' | 'off' */
  ...
  const PROBE_KEY = 'nasjicad.glprobe2'; /* {renderer, ok, ms, msNet, tier, reason} */
  ...
  /* phase 5 (weak-device parity): the probe grades the GPU by its NET
     ...
     gesture event. The gross ≥ BENCH_PASS_MS reject to 2D is unchanged. */
  ...
  let overdrawN = 1;                     /* throttle harness: N× the scene */
  const gov = {
  ...
  const runProbe = () => {
    ...
    try {
      ...
      /* then time until the minimum stops improving (≤24 frames): a ramp
  ...
  const govRebench = () => {
    ...
    govArmStepUp();                      /* C→B may keep climbing to A */
  };
    ...
    if (gov.presents % every === 0) {
      ...
      if (effTier() === 'A') {
        /* tier A never steps down on marginal or one-off samples — the
           ...
           miss — three samples running past 4× budget, the shape of a
        ...
        /* below A a synced sample at twice the budget is a real miss —
           ...
           used to demote B→C on an RTX in a single pan). Same strike
   ...
   * outer vertex; then O(n²) ear removal, with a forced clip so degenerate
   * input still terminates. Hatch boundaries are small (≤ a few hundred
   * points after tessellation), so O(n²) is nothing at build time. */
  const polyArea2 = (P) => {
    let a = 0;
  ...
  /* outer: [{x,y}] (no repeated closing point); holes: array of loops.
     Returns a flat Float32Array [x,y × 3 per triangle], or null. */
  ...
  /* phase 3: a SOLID hatch (and the '@bg' wipeout, which IS a SOLID hatch
     of '@bg' per SPEC3 §23) is GL-drawable. Pattern hatches are not: the
  ...
  /* insert placement 2x3: world = M·local + T (insertXform math, doubles) */
  ...
  const takeCycle = () => {
    ...
    /* phase 4 fix: an unhinted cycle ALWAYS sweeps. The old 'skip' arm
       ...
       pedit/grips commit shape (pushUndo → in-place mutate → render →
  ...
  const render = (gestureQ, tint, xf) => {
    ...
    /* view matrix in JS doubles: clip = M2·R(twist)·A·world + O(A·anchor)
    ...
    /* device pixels per world unit — the unit the rasterizer works in, so
       ...
       affine stretches every run by √|det|, so coverage follows it. */
    ...
    for (const [sx, sy] of [[cx0, cy0], [cx1, cy0], [cx0, cy1], [cx1, cy1]]) {
      ...
      if (xf) {
        /* item boxes are pre-transform world: cull against the view's
           pre-image through A⁻¹ (a degenerate A draws everything) */
    ...
    /* phase 5 throttle harness: N× overdraw ≈ a GPU N× weaker. Off (1) by
  ...
  /* the 2D-owned remainder (the engine's scene bitmap) as a textured quad
     ...
     composite's "scene over GL" layering. Corners run capture-inverse →
     world → current-forward in CPU doubles, so the text layer tracks the
  ...
  const drawRemainder = (rem, xf) => {
    /* phase 4 (zero flashing): a stale remainder never blanks the text
       ...
       flash was −59% of screen ink for a whole gesture. */
  ...
  /* the settled composite: the GL frame INTO the 2D canvas, under the
     ...
     gestures onto the DOM canvas to dodge a webgl→2d drawImage measured at
  ...
  const DRAG_LEFTOVER_CAP = 2000;        /* callers preview ≤300 of them */
  ...
  (() => {
    const ops = Nasj.docOps;
    /* commits whose mutation set is provably (selection ∪ wrapped ops):
  ...
  Nasj.glscene = {
    ...
    /* phase 5 throttle harness (debug only, never persisted): draw the
       scene N times per frame ≈ a GPU N× weaker. 1 = off, zero cost. */
    setOverdraw(n) {
