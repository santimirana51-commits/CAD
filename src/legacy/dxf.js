/* pixelbay CAD — dxf.js  (SPEC2 Â§17). Owner: FILEIO.
...
(() => {
  ...
  const aiSelectionFromPairs = (pairs) => {
    ...
    for (const [code, value] of pairs || []) {
      ...
      else if (app === 'PIXELBAY CAD' && code === 1000) marker = value === 'AI_SELECTION_V1';
      else if (app === 'PIXELBAY CAD' && marker && code === 1071) {
  ...
  /* ------------------------------------------------------------------ *
   * DXF text control codes (SPEC2 Â§17).
   * Import: %%d -> Â° (U+00B0), %%c -> Ã˜ (U+00D8), %%p -> Â± (U+00B1),
   * %%u / %%o underline/overline toggles stripped, %%% -> %, %%nnn ->
   * font char nnn, \U+XXXX -> the real character.
   * Export: Â° Ã˜ Â± are written back as %%d/%%c/%%p for compatibility with
   ...
   * the codepoint itself (initial Øª is its own character), so every reader
  ...
  AR_FORMS[0x0621] = [0xFE80];                       /* Ø¡ never joins       */
  ...
  Object.keys(LAM_ALEF).forEach((cp) => {
    LAM_ALEF[cp].forEach((fm) => {
      AR_UNSHAPE[fm] = 'Ù„' + String.fromCharCode(cp);
    });
  });
  const unshapeArabic = (s) => (/[ï­-ï»¼]/.test(s)
    ? s.replace(/[ï­-ï»¼]/g, (ch) => AR_UNSHAPE[ch.charCodeAt(0)] || ch)
    : s);

  /* BRACKET MIRRORING (UAX#9 N0-lite). The industry standard positions brackets with its
     bidi but never mirrors the glyph, so an RTL "(ÙƒØ°Ø§)" draws inside-out.
  ...
  const normalizeIncoming = (t) => (/[ï­-ï»¼]/.test(t)
  ...
  /* ------------------------------------------------------------------ *
   * Nested-insert flattening (import support). SPEC2 Â§15 forbids nested
  ...
  /* The two CONJUGATE semi-diameters of the image of a round curve, turned
     ...
     hairline (radius 3.1 million, sweep 1.8 microradians, under 600Ã—800)
  ...
  const xformBlockChild = (src, base, ins, newId) => {
    ...
    switch (e.type) {
      ...
      default:
        return null;                             /* insert / unknown â†’ caller */
    }
    return e;
  };
  ...
  /* The writer is a GENERATOR: it yields at natural checkpoints (every
     ...
     drawing's save never holds the thread (SPEC Â§18; engine's
     LOAD_SLICE_MS idiom). */
  function* exportDocGen(doc, opts) {
    ...
    /* one map, not a per-entity linear find — 738 layers Ã— 1.6 million
    ...
    /* Annotations are written as their exploded parts (SPEC2 Â§17), and the
    ...
    for (const nm of ['ACAD', 'PIXELBAY CAD']) {
      w(0, 'APPID'); w(5, handle());
      w(100, 'AcDbSymbolTableRecord'); w(100, 'AcDbRegAppTableRecord');
      w(2, nm); w(70, 0);
    }
    ...
    const write3dPolyline = (ent, pts, closed) => {
      ...
      w(1001, 'PIXELBAY CAD'); w(1000, 'Z3D');
    ...
    const writeEntity = (ent) => {
      ...
      if (aiSelectionNumber(ent.aisel) && ent.type === 'hatch') {
        ...
        w(1001, 'PIXELBAY CAD'); w(1000, 'AI_SELECTION_V1'); w(1071, ent.aisel);
        return true;
      }
      ...
      switch (ent.type) {
        ...
        case 'text': {
          ...
          const complex = /[Ö-à£¿ï¬-ï»¼]/.test(String(ent.str == null ? '' : ent.str));
          if (ent.mt || complex) {
          ...
          /* justification (SPEC2 Â§15 optional fields): 72/73 + 11/21 + 41 */
          const ha = (ent.ha === 1 || ent.ha === 2 || ent.ha === 4) ? ent.ha : 0;
          const va = (ent.va === 1 || ent.va === 2 || ent.va === 3) ? ent.va : 0;
        ...
        case 'hatch': {
          ...
          if (!solid) {
            ...
            /* true pattern line spacing (imported ent.sp) wins; a native
               hatch fits the pattern to its shape — sqrt(bbox area)/30 Ã—
        ...
        case 'mline': {
          ...
          const scale = isNum(ent.scale) && ent.scale !== 0 ? ent.scale : 1;
          /* the app's element offsets: STANDARD Â±0.5, shifted by the
        ...
        case 'leader':
        case 'table': {
          /* export as exploded primitives (SPEC2 Â§17) — the pre-pass
  ...
  const importText = (text) => {
    ...
    try {
      ...
      const ltIn = (name) => {
        ...
        return null; /* Continuous / unknown â†’ ByLayer default */
      };
      ...
      /* ---------------- hatch boundary loops (SPEC2 Â§17) ----------------
      ...
      /* multi-loop policy (SPEC2 Â§17): disjoint loops each become their own
        ...
        switch (type) {
          ...
          case 'HATCH': {
            ...
            /* pattern definition line (53 angle / 45,46 offset) carries the
               ...
               renderer matches the industry standard's density (SPEC2 Â§15) */
            let sp = null;
            if (!solid) {
              ...
              /* sp becomes the renderer's BASE spacing, which it multiplies
                 ...
                 family is not 1Ã— (ANSI32's is 3Ã—) must hand back the base,
          ...
          case 'DIMENSION': {
            ...
            /* group 2 names the anonymous *D block holding the rendered
               dimension geometry, defined in world coordinates â†’ identity
               insert. Missing block def â†’ counted as skipped (as before). */
          ...
          case 'LEADER': {
            ...
            if (pts.length < 2) return null;      /* invalid leader â†’ drop */
            return Object.assign(baseProps(q), {
      ...
      /* all block defs known â†’ expand nested inserts inside them */
