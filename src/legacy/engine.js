/* pixelbay CAD — engine.js
 * Viewport, rendering, pointer/osnap pipeline (SPEC Â§8). Owner: ENGINE.
...
(() => {
  ...
  /* Colors are the exact values from SPEC Â§2/Â§8 (tokens.css mirrors them). */
  const COL = {
    bg: '#212830',
    gridMinor: '#2a323c',
    gridMajor: '#333d49',
  ...
  Nasj.settings = {
    /* polar tracking ships ON, as the industry standard's does: the green alignment
       vector and its "Polar: d < aÂ°" readout appear while drawing and
    ...
    /* what a trackpad's two-finger scroll does on the drawing: 'auto' pans
       ...
       'never' zooms every scroll the way a mouse wheel does. OPTIONS â–¸ User
       Preferences keeps it; app.js copies the saved value here at boot. */
    trackpadPan: 'auto',
    /* Drafting Settings â–¸ Snap and Grid. gridSize stays the one number the
    ...
    zoomFactor: (() => {
      try { const v = +localStorage.getItem('pixelbay.zoomfactor'); return (v >= 3 && v <= 100) ? v : 60; }
      catch (_) { return 60; }
    })(),
    ...
    /* â–¸ Polar Tracking */
    /* â–¸ Dynamic Input — the industry standard ships both prompt lines on */
    ...
    /* CCONSTRAINTFORM: the form a new dimensional constraint takes —
       ...
       (behaves like a dimension and plots). The Parametric â–¸ Dimensional
  ...
  const fmtAng = (deg, degSign) => {
    ...
    switch (clampI(u.aunits, 0, 4)) {
      ...
      default: return d.toFixed(p) + (degSign ? 'Â°' : '');
    }
  };
  ...
  /* 12.5 Â· 1'6" Â· 1'-6 1/2" Â· 6" Â· 1' Â· 6 1/2 Â· 1/2  â†’  drawing units */
  const parseLen = (str) => {
    ...
    if (s) {
      const fr = /^(?:(\d+)[\s-]+)?(\d+)\s*\/\s*(\d+)\s*["â€]?$/.exec(s);
      const wh = fr ? null : /^(\d+(?:\.\d+)?)\s*["â€]?$/.exec(s);
      if (fr) {
        if (!(+fr[3])) return null;
  ...
  /* 45 Â· 45.5d Â· 45d12'30" Â· 50g Â· 0.7854r  â†’  degrees, unit conversion only */
  const parseAngVal = (str) => {
    const s = String(str == null ? '' : str).trim();
    if (!s) return null;
    const dms = /^([-+]?\d+(?:\.\d+)?)\s*d\s*(?:(\d+(?:\.\d+)?)\s*'\s*(?:(\d+(?:\.\d+)?)\s*["â€])?)?$/i.exec(s);
    if (dms) {
  ...
  /* SPEC3 Â§23: UI visibility flags — engine exposes only the object;
  ...
  /* ---------------- platform: the Mac ----------------
     ...
     labels follow: Nasj.keyLabel('Ctrl+Shift+S') prints â‡§âŒ˜S on a Mac and
  ...
  /* html.mac on every Mac (Safari and Chrome on the web build too: the
     labels read âŒ˜ there as well); html.mac-app only in the desktop app,
  ...
  const MAC_MOD = { ctrl: 'âŒ˜', shift: 'â‡§', alt: 'âŒ¥' };
  const MAC_MOD_ORDER = ['alt', 'shift', 'ctrl'];   /* âŒ¥ â‡§ âŒ˜ — the Mac's own order */
  Nasj.keyLabel = (s) => {
    s = String(s == null ? '' : s);
  ...
  /* SPEC3 Â§23: the active background color — model canvas vs paper sheet
   ...
   * Angles in radians. Top view = azimuth 0 / elevation +90Â°, which projects
   * exactly like the 2D mapping (points on z=0). Viewer direction for a
   * given azimuth/elevation is (sin azÂ·cos el, âˆ’cos azÂ·cos el, sin el). */
  const DEG = Math.PI / 180;
  const ISO_EL = Math.atan(1 / Math.SQRT2); /* 35.264…Â°: true isometric tilt */
  ...
  /* world â†’ view plane (rotate about Z by azimuth, tilt by elevation, drop
  ...
  /* how square a face is to a light: |nÂ·l|, 0..1; two-sided, since a 3D
  ...
  /* ---------------- view twist (SPEC2 Â§18: the saved view's rotation) ----
  ...
  /* The view-frame box of an entity, measured from the geometry rather
     than from its world box. Turning a box GROWS it — a plan drawn at 45Â°
  ...
  /* picking in 3D: the point on the CONSTRUCTION PLANE under the screen
     ...
     O + aÂ·U + bÂ·V, the projection is affine in a and b, and the screen
  ...
  const normAz = (a) => { /* wrap into (âˆ’Ï€, Ï€] */
    a %= TAU;
    if (a > Math.PI) a -= TAU;
    if (a < -Math.PI) a += TAU;
    return a;
  ...
  /* ViewCube curved arrows: spin the view 90Â° about the vertical axis (in
  ...
  /* ---------------- perf caches (SPEC Â§8 — behavior-neutral) ----------------
  ...
  /* A block reference expands to every flattened child of its definition —
     ...
     (SPEC Â§8 holds: a snap beyond the aperture is unreachable by
  ...
  /* per-entity snap points, same keying (SPEC Â§8: osnap results identical) */
  ...
  /* prims whose bbox crosses the (w Â± tol) cursor box — keeps the pairwise
     intersection test O(prims-near-cursorÂ²), not O(entity-sizeÂ²) */
  ...
  /* entities whose snap box may intersect the (xÂ±tol, yÂ±tol) query box.
  ...
  const drawHatchPattern = (c2d, ent, color, alpha0) => {
    ...
    if (pat) {
      ...
      /* imported hatches carry the true pattern line spacing (ent.sp,
         ...
         spacing = sqrt(bbox area)/30 Ã— scale, so a 5-unit tile and a
      ...
      /* A SOLID FILL IS INK, NOT A WASH. It used to go on at .45 alpha, so a
         ...
         SPEC3 Â§23: a SOLID hatch of '@bg' is a wipeout — fully opaque
  ...
  /* POCHÃ‰ MODE: closed polylines on layers matching /^A-WALL/ fill solid
  ...
  /* No save/restore here: 6k bars Ã— the state-stack round trip cost more
  ...
  const drawText = (c2d, ent, color, alpha) => {
    /* imported CAD text carries ha/va (DXF justification); native pixelbay CAD
       text has neither and renders exactly as before (SPEC2 Â§15). */
    ...
    if (st && st.vertical) {
      ...
      /* MTEXT: p is the top-left corner; lines stack down at 1.6 Ã— height */
  ...
  /* the PDMODE glyph, shared with the Point Style dialog's previews:
     base 0 dot / 1 nothing / 2 + / 3 Ã— / 4 tick; +32 rings a circle
  ...
  const drawArcText = (c2d, ent, color, alpha) => {
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      /* canvas y runs down, so world angle a becomes screen rotation -a;
         the baseline lies 90Â° clockwise (in world) from the up direction */
      c2d.save();
  ...
  const strokeEntity = (c2d, ent, color, lineWidth, dash, alpha, primary, tileOK, near) => {
    ...
    /* SPEC3 Â§23: the primary pass applies the entity's resolved linetype;
    ...
    c2d.globalAlpha = alpha;
    c2d.strokeStyle = color;
    /* hatch outline stays a hairline in the primary pass (SPEC2 Â§15) */
  ...
  /* GRID LAYER CACHE. Stroking the grid live every composite reintroduced
     GPU anti-alias jitter (Â±1 per channel between identical frames), which
  ...
  /* grips by intent, tier 2â†’3 boundary: up to this many selected entities
     ...
     table 35,273 â†’ 507 drawn squares) — the boundary is set where the
     ...
     selection-event table build is still â‰¤15ms. */
  ...
  const drawGrid = (c2d) => {
    ...
    /* the adaptive x10 step used to snap a whole line family in at the
       6px cutoff — fading minors over 6â†’12px lands the family invisible
  ...
  /* paper-space preview: dark desk, white A3 sheet, dashed margin, viewport
     rect with the model scaled-to-fit (SPEC2 Â§15). No grid/axes. */
  ...
  /* model â†’ paper â†’ screen collapses to one affine, so the ordinary 2D
   ...
   * every call: a Modelâ†’Layout click on a heavy drawing paid 700ms+, and
  ...
  const startSceneJob = (sig) => {
    ...
    /* HOW BIG AN ANSWER IS WORTH ASKING FOR. The probe costs one Set entry
       per entity the view touches — measured at ~0.15Âµs each, so 48 000 is
  ...
  const sceneFadeIn = () => {
    ...
    /* AND NOTHING TO EASE WHEN NOTHING SNAPS. The ease exists for the
       ...
       picture's edge energy (888 â†’ 898 on BLOCKS at a deep view). Easing
  ...
  /* quantised to WHOLE DEVICE PIXELS: a fractional pad puts the crop
     ...
     the bottom scanline flickers Â±1 between otherwise identical frames */
  ...
  const drawEntities = (c2d, doc, hoverBakedId, from, to, cutoff, deadline, listIn) => {
    ...
    for (let idx = from | 0; idx < z; idx++) {
      ...
      /* SPEC3 Â§23: with LWT on, stroke width = max(1.4, lw*2.2) px */
  ...
  const glRemainder = (ok) => ({
    ...
    /* phase 4: even when ok is false (content moved), the bitmap plus its
       ...
       beats presenting nothing (the âˆ’59% first-gesture flash) */
  ...
  /* OPTIONS â–¸ Colors… (and Grip Colors / Visual Effect Settings): the
  ...
  Nasj.render = () => {
    ...
    /* ============================================================ *
     * UNDEFEATABLE ZOOM (SPEC Â§8 spirit). The scene cache used to die on
    ...
    if (!sameSig(sceneState, sig)) {
      ...
      if (!crispPending2 && (glLive ? glGesture : blitMode) &&
        ...
        /* glscene phase 3: below tier A the gesture frame PRESENTS through
           ...
           no per-notch webglâ†’2d drawImage sync. At tier A, and in 2D-only
  ...
  /* the rotate gizmo's rings as the screen sees them: each ring lies in
     ...
     (u Ã— w = the axis, so the ring's angle runs the right hand's way).
  ...
  const GC_GLYPH = {
    horizontal: 'â”', vertical: 'â”ƒ', parallel: 'âˆ¥',
    perpendicular: 'âŠ¥', coincident: 'â—‡', concentric: 'â—Ž',
    collinear: 'â‰¡', tangent: 'â—‹', equal: '=', fix: 'âŠ•',
    smooth: 'âˆ¿', symmetric: 'â‹ˆ',
  };
  /* the industry standard's dimension-styled parameter, in screen points: âœ• at each
  ...
  /* the actions bound to a parameter, named for its âš¡ bar */
  ...
  const drawBeditChrome = (c2d, doc, only) => {
    ...
    c2d.font = '11px "Segoe UI", sans-serif';
    c2d.textBaseline = 'middle';
    /* the base point the session carries (BPARAMETER Basepoint): âŠ• */
    if (inEd && R.base) {
  ...
  /* TRIM's red âœ• on the cursor: what is under it is about to be removed */
  ...
  /* canvas fallback for the "Polar: d < aÂ°" tooltip when dynamic input is off */
      ...
      for (const ent of ui.preview) {
        if (ent.type === '@xfblit') {
          /* THE UNIFIED GHOST SOURCE, 2D EDITION. A whole-selection drag's
             ...
             one). Three bitmap corners map captureâ†’worldâ†’affineâ†’screen
  ...
  /* ---------------- grips (SPEC Â§8/Â§9 grip editing) ----------------
   ...
   *   1 (â‰¤ GRIPOBJLIMIT):     every grip — the untouched path above.
   *   2 (â‰¤ GRIP_ENV_LIMIT):   reduced per-entity grips — ends, centers,
   ...
   * entity's full grips (hover promotion, â‰¤1 frame); they retire when the
   ...
   * temporarily swapping the viewport's worldâ†’screen mapping — same trick as
  ...
  Nasj.plotRender = (opts) => {
    ...
    if (doc) {
      ...
      c2d.clip();
      if (opts.upsideDown) {                 /* rotate the whole plot 180Â° */
        c2d.translate(CW, CH);
        c2d.rotate(Math.PI);
      }

      /* swap the worldâ†’screen mapping under the shared draw pipeline */
    ...
    if (opts.plotStamp) {                    /* 8pt gray stamp, bottom-left */
      ...
      c2d.fillText(name + ' — ' + when + ' — pixelbay CAD' + (window.NASJ_VERSION || ''), margin * ppm + 2, CH - Math.max(2, margin * ppm * 0.25));
      c2d.restore();
    }
  ...
  /* UNTANGLE: a crossing loop tears a fill — a wedge out of the 2D
     ...
     trace, a snapped boundary), so a few O(nÂ²) passes settle it. Works in
  ...
  Nasj.makeBoundaryTracer = () => {
    ...
    const buildRaster = (box, allowGl) => {
      ...
      /* the view is already a picture: 1024 is enough to pick a region,
         and getImageData of 2048Â² was ~200ms of the first HATCH hover */
    ...
    const trace = (R, world) => {
      ...
      const MIN_ISLAND = 12;                            /* pxÂ² — grid noise cut */
    ...
    const at = (world, opts) => {
      ...
      for (let i = 0; i < 4; i++) {
        /* Ã—4 a step so the raster stays proportioned to the region — a
  ...
  const EL_MAX = 89.9 * DEG;    /* smooth elevation clamp 0..89.9Â° */
  ...
  /* ---------------- the wheel: mouse, trackpad, pinch ----------------
     ...
     only by their shape, so the shape is what is read:
       PINCH  — ctrlKey set (Chromium's own convention for a trackpad
                ...
                exp(-deltaY * 0.01), clamped to Â±25% an event, no steps;
       ...
       WHEEL  — the rest: the mouse's notches (integer Â±100/Â±120/Â±53
     ...
     'never') overrides the guess; OPTIONS â–¸ 3D Modeling's "Reverse mouse
