/* pixelbay CAD — commands.js (owner: INTERACT)
   Command line, parsing, autocomplete, history, dynamic input, clipboard. SPEC Â§10. */
(() => {
  'use strict';
  ...
  /* Prompt label with clickable [Keyword] chips (spec Â§0.2: each bracketed
  ...
  /* ---------------- command table ---------------- */
  const CMDS = [];
  /* opts: { cat: help-group name, edit: true â†’ the command changes geometry,
     icon: NasjIcons name for the suggest popup / search (SPEC3 Â§21),
     trans: true â†’ may run transparently with a ' prefix (spec Â§0.1) } */
  ...
  const showHelp = () => {
    ...
    print('Coordinates: X,Y absolute Â· @DX,DY relative Â· @D<A polar (degrees).');
    if (N.paper) {
      print('Paper space: drawing lands on the sheet — double-click a viewport to work in the model.');
    }
  };

  /* ---------------- v3 immediate commands (SPEC3 Â§22) ---------------- */
  ...
  def('SETBYLAYER', [], startTool('setbylayer'), 'Give the selection back to its layerâ€™s colour, linetype and lineweight', { cat: 'Modify', edit: true, icon: 'set-bylayer' });
  ...
  }, 'The blockâ€™s table of value sets — one row is one configuration',
  ...
  }, 'Manage the blockâ€™s visibility states', { cat: 'Blocks', icon: 'block-edit' });
  def('BTESTBLOCK', [], () => {
  ...
  def('DSETTINGS', ['DS', 'SE'], () => {
    ...
    print(`  Ortho: ${onOff('ortho')}   Polar: ${onOff('polar')} (increment ${st.polarAng || 90}Â°)   Osnap: ${onOff('osnap')}   Otrack: ${onOff('otrack')}`);
  ...
  /* Express Tools â–¸ Modify Text — the industry standard's five, the flyout's own order */
  ...
  def('SUNPROPERTIES', ['SUN'], () => {
    ...
    print('Sun Properties opened. Azimuth ' + p.azimuth.toFixed(4) +
      'Â°, altitude ' + p.altitude.toFixed(4) + 'Â°.');
  ...
  /* Express Tools â–¸ Blocks — the nested-object family */
  ...
  /* Express Tools â–¸ Text — remote text and the mask pair */
  ...
  /* the Insert â–¸ Import panel: a file brought INTO this drawing, not opened
  ...
  def('QUIT', ['EXIT'], fileAction('quit'), 'Leave pixelbay CAD, offering to save every changed drawing',
  ...
  def('ABOUT', [], () => {
    print(('pixelbay CAD' + (window.NASJ_VERSION || '')) + ' — professional 2D CAD for Windows.');
    print('DWG/DXF files, dimensions, hatches, layouts, groups and PDF plotting.');
  }, 'About pixelbay CAD', { cat: 'Utility', icon: 'qnew' });
  def('ACADINFO', [], () => {
    const doc = N.doc || {};
    print(('pixelbay CAD' + (window.NASJ_VERSION || '')) + ' — drawing "' + (doc.name || 'Drawing') + '": ' +
  ...
  def('ARRAYEDIT', [], () => {
    print('Associative arrays are not in this release — the arrays pixelbay CAD places are plain copies.', 'err');
  ...
  /* ---------------- the industry standard's compatibility names ----------------
     ...
     ones whose features pixelbay CAD does not ship say so plainly instead of
  ...
  compat('an architectural add-on command; not part of pixelbay CAD.',
  ...
  compat('an internal dimension helper of another program; not part of pixelbay CAD.',
  ...
  compat('a markup assist command; not part of pixelbay CAD.',
  ...
  /* Insert â–¸ Reference. An external reference is a drawing on disk shown
  ...
  compat('a scripting helper; not part of pixelbay CAD.', ['ADDVARS2SCR']);
  ...
  const ACT_KEY = 'pixelbay.actmacros';
  const actLoad = () => {
  ...
  const execute = (raw) => {
    ...
    let c = INDEX[tok];
    if (!c) {
      /* spec Â§0.1: AutoCorrect instead of hard-failing (wrench menu toggle).
    ...
    c.run();
    /* one-shot prefeed (SPEC3 Â§22/Â§24): "CIRCLE 2P", "ARC C", "VIEWSAVE name" —
  ...
  const rePt3 = new RegExp(`^${NUM}\\s*,\\s*${NUM}\\s*,\\s*${NUM}$`); /* x,y,z (spec Â§0.4) */
  ...
  /* commands that may run inside another command with a ' prefix (spec Â§0.1) */
  ...
  const routeText = (s) => {
    ...
    if (mode !== 'none') {
      let m;
      /* a Mac's Arabic keyboard layout types the digits as Ù Ù¡Ù¢… and the
         comma as "ØŒ" (Windows' layout types 0-9 and ","), so "0,0" typed
         there never read as a point. Coordinates and numbers read either
         way; the text fed to a prompt stays as typed. */
      const n = s.replace(/[Ù -Ù©]/g, (c) => String.fromCharCode(c.charCodeAt(0) - 0x0630))
        .replace(/ØŒ/g, ',').replace(/Ù«/g, '.');
  ...
  const matchCmds = (t) => {
    ...
    if (cli.midSearch !== false) { /* spec Â§0.1: typing SET offers DSETTINGS too */
  ...
  /* ---------------- dynamic input (industry-standard-style, F12) ----------------
     ...
     the next; Enter/Space submits; ArrowDown / the âŒ„ opens keyword options.
     While polar tracking is locked the bubble reads "Polar: d < aÂ°". */
  let dynEls = null;
  let lastPtr = null;
  ...
  const dynMode = () => {
    ...
    if (aw === 'point') {
      ...
      /* Pointer Input Settings â–¸ Format: a second point normally reads as
  ...
  const dynNum = (s, fb) => {
    const t = String(s).replace(/[Â°<]/g, '').trim();
  ...
  const submitDyn = () => {
    ...
    if (mode === 'wh') {                    /* RECTANG length Ã— width */
  ...
  const updateDyn = () => {
    ...
    if (track && mode === 'da') {
      ...
      const ang = track.axis ? track.axis
        : (U() ? U().fmtAng(track.ang * DYN_DEG, true)
          : Math.round((((track.ang * DYN_DEG) % 360) + 360) % 360) + 'Â°');
    ...
    el.box2.style.display = 'block';
    setLive(el.f2, 2, U() ? U().fmtAngVal(disp) : Math.round(disp) + 'Â°');
    if (Math.abs(th) > 0.02) {
  ...
  N.clipboard = {
    ...
    /* Make the clipboard's dependencies real in `doc` before a paste lands:
       ...
       rule for cross-drawing paste). Returns the sourceâ†’target layer-id
  ...
  N.cmd = {
    ...
    /* SPEC2 Â§16 + SPEC3 Â§22: command metadata (now includes icon) */
  ...
  N.cmdInit = () => {
    ...
    print(('pixelbay CAD' + (window.NASJ_VERSION || '')) + ' — type a command or press F1 for help.');
  };
})();
