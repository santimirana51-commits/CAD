/* img2dwg — nasj-pwa.js: what makes the web build of pixelbay CAD installable.
 *
 * Copied by scripts/sync-cad.mjs into public/pixelbay/js/ and named last
...
(() => {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    if (window.Nasj && typeof window.Nasj.toast === 'function') {
      window.Nasj.toast('pixelbay CAD is installed — open it from your apps');
    }
  });
  ...
  if ('serviceWorker' in navigator) {
    const register = () => {
      ...
      navigator.serviceWorker.register('/cad-sw.js' + (stamp ? '?v=' + stamp : ''), { scope: '/cad', updateViaCache: 'none' })
        .then(() => askVersion())
        .catch((err) => console.warn('[pixelbay] service worker not registered:', err && err.message ? err.message : err));
    };
  ...
  const render = () => {
    ...
    el.querySelector('.act').title = isIOS && !pwa.canInstall
      ? 'Safari: tap Share, then "Add to Home Screen" — pixelbay CAD opens like an app'
      : 'Install pixelbay CAD on this device — it opens on its own, and works with no connection';
    el.classList.add('on');
