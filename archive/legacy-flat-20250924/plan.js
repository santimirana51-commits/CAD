/* Nasjicad — plan.js: a floor plan from a room layout, as real CAD.
 *
...
(function (root) {
  ...
  const MIN_CELL_AREA = 1.0;         /* m²: a sliver the boundary leaves is dropped */
  ...
  /* the intersection of two lines (p + s·u, q + t·v) or null when parallel */
  const lineX = (p, u, q, v) => {
    const d = cross(u, v);
  ...
  const kindOf = (r) => {
    ...
    if ((!k || k === 'other' || k === 'room') && r.name) {
      ...
      if (/عرض|محل|مبيعات|تليفون|هاتف|shop|retail|showroom|sales|tienda|magasin|商店|店舗/.test(name)) return 'shop';
      if (/مخزن|تخزين|storage|store|almacén|仓库|倉庫/.test(name)) return 'store';
      if (/مكتب|office|bureau|oficina|办公室/.test(name)) return 'office';
  ...
  const buildCells = (rooms, plot, notes, build, W, H, program) => {
    ...
    for (const r of rooms) {
      ...
      if (!program?.some(p=>p.id===r.id) && !r.outdoor && meant > EPS && (area < 0.5 * meant || (meanWidth < 1.6 && meanWidth < 0.7 * Math.min(r.w, r.h)))) {
        ...
        r.lost = Math.round((1 - area / meant) * 100);
        notes.push(r.name + ' lost ' + r.lost + '% of its area to the boundary (' + r1(area) + ' m² of ' + r1(meant) + ' m² remain)');
      }
  ...
  const placer = (room, ri, pieces, tExt) => {
    ...
    /* try a w×d item against a wall; -> local frame {o,u,n} or null */
    const tryAt = (f, s, w, d) => {
    ...
    /* a w×h item centred at c (axis aligned) */
    const centred = (c, w, h) => {
  ...
  /* draw helpers in a local frame: L(x, y) = o + u·x + n·y */
  ...
  const label = (O, room) => {
    ...
    O.text({ x: c.x, y: c.y - hn * .9 }, area + ' m²', Math.min(ha,hn), LAYERS.text);
  };
  ...
  const compile = (layout, ctx) => {
    ...
    for (const r of rooms) {
      ...
      if (kindHas(k, ['wc', 'toilet', 'powder']) && r.area > 4.5) notes.push('FAULT: ' + r.name + ' is ' + r1(r.area) + ' m²; review the space allocated to a compact WC (planning preference, not a code maximum; preserve requested accessibility clearances)');
      else if (kindHas(k, ['bath', 'shower', 'ensuite']) && r.area > 10) notes.push('FAULT: ' + r.name + ' is ' + r1(r.area) + ' m²; review whether this much bathroom area is intended; size depends on fixtures and accessibility');
      ...
      else if (kindHas(k, ['stair']) && r.area > 25) notes.push(r.name + ' is ' + r1(r.area) + ' m²; review whether this much stair and landing area is intended');
      else if (kindHas(k, BEDROOMS) && r.area < 9) notes.push('FAULT: ' + r.name + ' is ' + r1(r.area) + ' m² - a bedroom needs 9 m² or more');
    ...
    if (onLand) notes.push('Indoor footprint: ' + r1(indoorArea) + ' m² of the full ' + r1(fullLandArea) + ' m² site (' + Math.round(indoorArea/fullLandArea*100) + '%).');
  ...
  const describe = (layout) => {
    ...
    const parts = rooms.filter((r) => r && typeof r === 'object').map((r) =>
      String(r.name || r.kind || r.id || 'room') + ' ' + r3(Number(r.w) || 0) + ' × ' + r3(Number(r.h) || 0) + ' m');
