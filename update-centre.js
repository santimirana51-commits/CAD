...
(() => {
  ...
  const boot = () => {
    ...
    const symbol = make('span', 'nu-symbol', 'â†»'); symbol.setAttribute('aria-hidden', 'true');
    ...
    const heading = make('h2', '', 'pixelbay CAD updates'); heading.id = 'nu-heading';
    const close = make('button', 'nu-close', 'Ã—'); close.type = 'button'; close.setAttribute('aria-label', 'Close updates');
    ...
    const render = () => {
      ...
      button.title = 'Current: v' + state.current + ' Â· Latest: ' + (state.latest ? 'v' + state.latest : 'not checked') + '. Open updates';
      ...
      status.textContent = saving ? 'Saving your drawings…' : n.installing ? 'Installing the update…' : n.downloading ?
        'Downloading inside pixelbay CAD: ' + Math.round(Number(n.percent) || 0) + '%' : n.ready ? 'Downloaded and ready to install.' :
    ...
    const check = () => {
      ...
      pending = (async () => {
        ...
        try {
          ...
          const r = await fetch('/pixelbay/version.json?check=' + Date.now(), { cache: 'no-store', signal: ctl.signal });
