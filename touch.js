/* pixelbay CAD — touch.js. Fingers on the drawing.
 *
 ...
 * command line, the prompt's [options] as chips, a âŠž that opens the
 * command palette and a â‹¯ menu) and the html.coarse class that sizes the
 * chrome for fingers. The command line itself raises no system keyboard
 * on a tap — it is read-only until the âŒ¨ asks for one, or a prompt for
...
(() => {
  ...
  const init = () => {
    ...
    const typeText = (ch) => {
      const i = cmdInput();
      if (!i) return;
      i.value = ch === 'âŒ«' ? i.value.slice(0, -1) : i.value + ch;
      typed(i);
    };
    ...
    /* Enter with nothing running repeats the last command, as a keyboard's
       ...
       command is usually NEW (the toolbar's new-drawing button), and a âœ“
    ...
    const PAD_KEYS = ['7', '8', '9', 'âŒ«', '4', '5', '6', '@', '1', '2', '3', '<', '0', '.', '-', ','];
    const buildBar = () => {
      if (bar) return;
      const host = document.getElementById('viewport-container');
      if (!host) return;
      ...
      for (const k of PAD_KEYS) {
        pad.appendChild(mkBtn('tb-key' + (/\d/.test(k) ? '' : ' tb-sym'), k,
          k === 'âŒ«' ? 'Backspace' : k, () => typeText(k)));
      }
      pad.appendChild(mkBtn('tb-key tb-ok', 'âœ“', 'Enter', act.enter));
      /* the â‹¯ menu: what the row has no room for */
      ...
      const menuItem = (cls, glyph, label, run) => {
        ...
        b.innerHTML = '<span class="tb-mi-ic">' + glyph + '</span><span class="tb-mi-label"></span>' +
          '<span class="tb-mi-check">âœ“</span>';
      ...
      menu.appendChild(menuItem('tb-mi-kbd', 'âŒ¨', 'Keyboard', () => setKeyboard(!kbdWanted)));
      menu.appendChild(menuItem('tb-mi-hist', 'â‰¡', 'History', () => {
        const x = document.getElementById('cmd-expand');
        if (x) x.click();
      }));
      menu.appendChild(menuItem('tb-mi-opts', 'âš™', 'Options', () => { if (N().cmd) N().cmd.execute('OPTIONS'); }));
      /* app.css lays the row out right to left (row-reverse), so its
         last child — the prompt's option chips — reads leftmost, beside
         âœ“, and wraps to a line of its own above the buttons when the
         prompt has more options than the row has room for */
      const row = document.createElement('div');
      row.className = 'tb-row';
      row.appendChild(mkBtn('tb-btn tb-more', 'â‹¯', 'More — Commands, Keyboard, History, Options', act.more));
      ...
      row.appendChild(del);
      row.appendChild(mkBtn('tb-btn tb-undo', 'â†¶', 'Undo', act.undo));
      row.appendChild(mkBtn('tb-btn tb-esc', 'âœ•', 'Cancel (Esc)', act.esc));
      row.appendChild(mkBtn('tb-btn tb-enter', 'âœ“', 'Enter — accept / repeat', act.enter));
    ...
    /* the â‹¯ menu closes on a tap anywhere else */
    ...
    /* ---- the command line without the system keyboard ----
       ...
       ways: the âŒ¨ (the palette's search row, the bar's â‹¯ menu) keeps it
    ...
    const PAL_KEY = 'pixelbay.palette.recent';
    const PAL_RECENT = 8;
    const PAL_SECTIONS = [
    ...
    const buildPalette = () => {
      ...
      sheet.innerHTML = '<div class="cp-head"><span class="cp-grab"></span>' +
        ...
        '<button type="button" class="cp-btn cp-kbd" title="Keyboard — type into the command line">âŒ¨</button>' +
        '<button type="button" class="cp-btn cp-close" title="Close">âœ•</button></div>' +
