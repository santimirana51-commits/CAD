...
(() => {
  ...
  /* the camera, in the shader: q = the point measured from the target.
     U = rÂ·q, V = uÂ·q, D = dÂ·q. Orthographic: screen = c0 + rolled(U,V)Â·scale.
     Perspective: the same with (U,V) scaled by dist/w, w = dist âˆ’ D — done
  ...
  const VS_CAM = `
    uniform vec3 u_anchor;              /* chunk anchor âˆ’ target */
  ...
  /* an instance: a 2Ã—2 placement (rotation Â· scale, the insert's) and a
  ...
  const cameraNow = (w, h) => {
    ...
    D0 -= pad; D1 += pad;
    if (!cam.persp) {
      /* z_ndc = (D1 âˆ’ D) / (D1 âˆ’ D0) Â· 2 âˆ’ 1: nearer (larger D) is smaller */
      cam.zA = -2 / (D1 - D0);
      cam.zB = 2 * D1 / (D1 - D0) - 1;
    } else {
