...
(() => {
  ...
  /* EXTRUSION: an xy ring (with optional hole rings) swept from z0 up by h.
     ...
     same rule — the side of an edge a→b faces (dy, −dx) — points out of the
     body on both. */
  ...
  const cylinder = (c, r, h, n, rTop) => {
    ...
    for (let i = 0; i < n; i++) {
      const j = (i + 1) % n;
      if (apex) {
        /* edges: chord (0), seam j→apex (1), apex→seam i (2) */
  ...
  const sphere = (c, r, n) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      /* bottom cap triangles: S, r0[j], r0[i] — edges S→r0[j] (meridian j), chord (parallel 1), r0[i]→S (meridian i) */
