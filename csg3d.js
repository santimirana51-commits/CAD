 ...
 *
 * The pipeline: faces â†’ triangles (ear clipping, holes bridged) â†’ BSP â†’
 * clipped fragments â†’ REBUILT faces. The rebuild is what keeps the result
...
(() => {
  ...
  const facesFromPolys = (polys) => {
    ...
    const verts = new Map();                /* key â†’ 3D vertex */
    ...
    for (const g of groups.values()) {
      ...
      const V = normV(crossV(n, U));        /* U Ã— V = n: CCW is about +n */
      const p2 = (v) => [dotV(v, U), dotV(v, V)];
      /* directed edges, opposite pairs cancelled */
      const eCount = new Map();             /* "ka>kb" â†’ count */
      const bump = (ka, kb) => {
        if (ka === kb) return;
      ...
      const outMap = new Map();             /* key â†’ [destKey,...] */
      ...
      /* rings â†’ faces: collinear points dropped, area signs sorted out */
      const built = [];
  ...
  const markSeams = (faces) => {
    ...
    const edges = new Map();       /* key â†’ [{fi, ei}] */
    faces.forEach((f, fi) => {
  ...
  const slice = (faces, p0, nrm) => {
    ...
    const o = vOf(p0);
    const boxFaces = (side) => {
      /* corners: o Â± uÂ·d Â± vÂ·d, out to o + nÂ·sideÂ·d */
