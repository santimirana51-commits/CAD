...
(() => {
  ...
  /* ================================================================
     ...
     picking cube):
     - six axis balls (Â±X Â±Y Â±Z) with hover highlight; a click glides
       the camera down that axis (Top/Bottom/Front/Back/Left/Right)
     - the Â±X/Â±Y beads sit on a ground ring; dragging the ring is a
       constrained turntable spin (azimuth only) that snaps near 45Â°
  ...
  const renderCube = () => {
    ...
    /* ---- ground ring in the z=0 plane; the Â±X/Â±Y beads sit on it ----
  ...
  if (vcCanvas && vcCtx) {
    ...
    vcCanvas.addEventListener('pointerup', (ev) => {
      ...
      if (drag && drag.moved) {
        /* a turntable spin released near a 45Â° stop settles onto it */
  ...
  /* curved arrows: spin the view 90Â° per click */
