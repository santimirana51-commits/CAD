/* Nasjicad — agent-panel.js
 * Renderer-side AI panel (AGENT-CONTRACT §3). Owner: PANEL.
 ...
 * (PANEL-CONTRACT §3, unchanged) — and replies with a NEUTRAL summary
 * (AGENT-CONTRACT §1: the model never learns how a drawing is made; the
...
(() => {
  ...
  const MAX_ROWS = 8;
  const EPS = 1e-9;
  /* AGENT-CONTRACT §2's documented default; replaced the moment the service
     reports the model it actually used on an event. */
  const MODEL_FALLBACK = 'deepseek-flash';

  /* Neutral tool-failure sentences. AGENT-CONTRACT §1: what goes back to the
   ...
   *   fit onto CLEAN_SIZE² white, letterboxed (never distorted),
   ...
   * 5632×3072 image/jpeg for "draw simple square"), because generateContent
   * has no output-format control. JPEG on hard black-on-white edges ALWAYS
   * rings: every line gets a grey halo, flat white picks up 8×8 blocking, and
  ...
  const plotFrame = (pts, box, force, alone) => {
    ...
    /* fold to (-90°, 90°]: the long edge horizontal, the plan upright */
  ...
  let updateGate = null;   /* a required update (app.js ▸ Nasj.updateNotice): the agent waits for it */
  ...
  /* a usage event, through the tracker (nasj-track.js ▸ Nasj.track): the
  ...
  const SETUP = {
    ...
    update: { ico: 'ai-spark', head: 'Update Nasjicad to keep drafting.',
      text: 'Install the required update in Nasjicad, then send your message again.',
  ...
  const paintCredit = () => {
    ...
    if(a.unlimitedAi){creditEl.querySelector('#ag-credit-amt').textContent='∞ AI';creditEl.setAttribute('aria-label','Unlimited AI is active');creditEl.title='Unlimited AI is active — open your account';return;}
  ...
  const renderChips = () => {
    ...
    else if(st.region?.aisel!=null)addChip('ai-vector',REFERENCE_LABELS.area+' '+st.region.aisel+(st.region.metres?' · '+st.region.metres:''),'Remove area reference',clearTarget);
    else if(st.attachment)addChip('ai-vector',st.attachment.label+(st.attachment.metres?' · '+st.attachment.metres:''),'Remove boundary reference',clearTarget);
  ...
  const API = {
    ...
    send(text) {
      ...
      if (updateGate) {
        showError('Nasjicad V' + updateGate.version + ' is required before the agent can draw. ' +
    ...
    /* a required update (app.js ▸ Nasj.updateNotice): {version, url, …}
