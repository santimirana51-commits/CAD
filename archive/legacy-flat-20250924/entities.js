/* Nasjicad — entities.js
 * Data model + geometry kernel (SPEC §7). Owner: ENGINE.
...
(() => {
    ...
    table: {
      ...
      props: (() => {
        ...
        for (const [k, fill, bold] of [['t', 'None', true], ['h', 'None', true], ['d', 'None', false]]) {
          ...
          p[k + 'H'] = { def: bold ? 0.6 : 0.55, min: 0.05 };   /* × row height */
          p[k + 'Clr'] = { def: 'ByBlock', str: true };
          p[k + 'Ang'] = { def: 0, min: -360, max: 360 };
          p[k + 'MargH'] = { def: 0.225, min: 0 };              /* × row height */
  ...
  Nasj.createDoc = (name) => ({
    ...
    currentLayerId: '0',
    entities: [],
    blocks: {},            /* SPEC2 §15: {[name]: {base:{x,y}, entities:[...]}} */
    views: [],             /* SPEC3 §23: [{name, scale, tx, ty}] named views */
   ...
   * every one is asserted in qa-eval-modify §U, which hashes the document
  ...
  /* deserialize validation: required finite geometry per type (SPEC §7) */
  ...
  /* hatch boundary validation (SPEC2 §15) */
  ...
  const repairGeom = (e) => {
    ...
    switch (e.type) {
      ...
      case 'image': /* raster attachment (IMAGEATTACH): lower-left p, size
                       w×h in world units, bitmap in doc.images[e.img] */
  ...
  Nasj.docOps = {
    ...
    resolveColor(doc, ent) {
      ...
      /* SPEC3 §23: '@bg' resolves to the active background (model vs paper) */
      if (col === '@bg') {
    ...
    /* SPEC3 §23: linetype resolves like color (ent.lt -> layer.lt -> continuous) */
    resolveLt(doc, ent) {
    ...
    /* SPEC3 §23: lineweight in mm (ent.lw -> layer.lw -> the Default).
       The Default is the industry standard's LWDEFAULT — Options ▸ Lineweight
    ...
    deserialize(json, name) {
      ...
      if (Array.isArray(d.layers)) {
        const lys = d.layers
          ...
          .map(l => {
            ...
            /* optional lt/lw (SPEC3 §23) — kept only when valid */
      ...
      /* block definitions (SPEC2 §15) — validate like entities, no nested inserts */
      doc.blocks = {};
      ...
      /* named views (SPEC3 §23) — validate each entry */
      doc.views = [];
  ...
  /* CCW sweep from a0 to a1.
     A whole turn is SPELLED OUT — a0 to a0 ± 2π, the form a full sweep
  ...
  const textWidth = (ent) => {
    ...
    /* CAD text — what an import made, and what carries no style at all —
       draws at the industry standard's cap height: 0.67 × that height per character, which
  ...
  /* MTEXT line spacing: 1.6 × the text height, the industry standard's single-ish spacing */
  ...
  const arrowHeadGeom = (G, seg, tip, dirIn, head, ALEN) => {
    ...
    switch (head) {
      ...
      case 'Datum triangle': case 'Datum triangle filled': {
        const w = ALEN * Math.tan(Math.PI / 6);   /* 60° at the tip (GD&T) */
      ...
      case 'Integral': {
        /* the ∫ stroke: two shallow arcs meeting at the point */
        const rot = Math.atan2(dir.y, dir.x);
  ...
  const dimGeometry = (ent, stIn) => {
    ...
    const tolOf = (v) => {
      ...
      switch (st.tol) {
        case 'Symmetrical': return { suffix: ' ±' + up };
    ...
    switch (ent.kind) {
      ...
      case 'diameter': {
        ...
        const t = dress(annoText({ x: 0, y: 0 }, label('Ø' + fmtVal(r * 2)), TH,
  ...
  /* The placement map of a reference: the definition's own coordinates → the
  ...
  /* The other direction: world → the definition's own coordinates. XCLIP
  ...
  /* AN ARC UNDER A SQUASHED PLACEMENT IS AN ELLIPSE. A reference that
     ...
     microradians, under 600×800) the point landed 580,000 definition units
  ...
  const conjAxes = (u, v) => {
    ...
    /* |P(t)|² peaks at this t: P(t0) and P(t0+π/2) are the axes themselves */
  ...
  /* Definition coordinates → the world, for one placement. The entities are
  ...
  /* The world → definition coordinates: exactly insertToWorld run backwards,
  ...
  /* MLEDIT end caps: ent.capS / ent.capE remember, as an angle CCW from the
     ...
     absent (or π/2) is the plain square end. */
  ...
  const entityBounds = (ent) => {
    ...
    switch (ent.type) {
      ...
      case 'insert': {
        ...
        /* FAST PATH: a square placement (rotation a multiple of 90°) of a
  ...
  /* a reversed ring numbers its edges the other way: the edge from corner
     i to i+1 runs between the new corners n−2−i and n−1−i */
  const flipHid = (hid, n) => {
    let out = 0;
  ...
  const mirrorEntity = (ent, a, b) => {
    ...
    if (ent.type === 'arc' || ent.type === 'arctext') {
      ...
      /* M∘(R(rot)·S) = R(2phi-rot)·S(sx,-sy) — exact mirrored placement */
      ent.rot = norm2pi(2 * phi - (ent.rot || 0));
