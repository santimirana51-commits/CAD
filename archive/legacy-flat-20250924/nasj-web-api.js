 ...
 *
 * Nasjicad's renderer (src/js/*.js) never touches Electron. It reaches its
...
(() => {
   ...
   * the action, then where it lands — ← Back to Website [N] — so the mark
  ...
  const dataUrlToBlob = async (dataUrl) => (await fetch(String(dataUrl))).blob();
  ...
  const SAVE_TYPES = [
    ...
    { description: 'Nasjicad Drawing', accept: { 'application/json': ['.njc'] } },
  ];
  ...
  const upFx = (() => {
    ...
    const spin = (on) => { root.classList.toggle('spin', on); if (on) els.pct.textContent = '⋯'; };
   ...
   * dwg-doc.js, the same call the desktop's File ▸ Open makes) bundled for
  ...
  const webOpen = (() => {
    ...
    const fail = (why, msg) => {
      console.warn('[nasjicad] open by address: ' + why);
      upFx.dismiss();
      toast(msg || FAIL);
  ...
  const printImage = (dataUrl, landscape) => new Promise((resolve) => {
    ...
    w.document.write(
      '<!DOCTYPE html><html><head><title>Nasjicad</title><style>' +
  ...
  const stream = async (body) => {
    ...
    let res;
    try {
      res = await postJson('/api/agent', Object.assign({ capabilities: ['cad-document-v1','cad-workspace-v1','cad-library-v1','plan-repair-v1','architectural-docs-v1','coordination-v1','reference-selection-v1','plan-workflow-v1'] }, convId ? { convId } : {}, body), ctl.signal);
    } catch (err) {
      finish();
      if (ctl.signal.aborted) return;
      emit(agentListeners, { kind: 'error', code: 'network',
        message: 'The agent could not be reached: ' + errMsg(err) });
      return;
    }
    const type = res.headers.get('content-type') || '';
    if (!res.body || type.indexOf('text/event-stream') < 0) {
      /* a JSON refusal (signed out, a proxy) or an HTML error page */
      let j = null;
      try { j = await res.json(); } catch (err) { j = null; }
      finish();
      emit(agentListeners, { kind: 'error', code: (j && j.code) || 'server',
        message: (j && (j.error || j.message)) || ('the agent failed (HTTP ' + res.status + ')') });
      return;
    }

    /* the round is over on one of three events; a stream that closes on
       none of them (the server fell over) must not leave the panel spinning */
    let settled = false;
    const dispatch = (e) => {
      if (!e || typeof e !== 'object') return;
      if (ctl.signal.aborted) return;
      if (e.kind === 'conv') convId = e.convId || convId;
      if (e.kind === 'done' || e.kind === 'error') settled = true;
      if (e.kind === 'tool-exec') {
        settled = true;
        if (typeof e.balanceMicro === 'number') account({ balanceMicro: e.balanceMicro });
        emit(toolListeners, { id: e.id, name: e.name, args: e.args });
        return;
      }
      emit(agentListeners, e);
    };
    const chunk = (text) => {
      for (const line of text.split('\n')) {
        if (line.indexOf('data:') !== 0) continue;    /* ": ping" and blanks */
        try { dispatch(JSON.parse(line.slice(5))); } catch (err) { /* a torn line */ }
      }
    };
  ...
  const accountGet = async () => {
    ...
    try {
      ...
      return { ok: false, code: 'network', error: errMsg(err) };
    }
    if (j && j.ok) account(j);
    return j || { ok: false, code: 'server', error: 'HTTP ' + res.status };
  };
  ...
  const byokStatus = (b, extra) => {
    const provider = (b && b.provider) || 'xai';
    const key = b && b.keyTail
      ? { set: true, source: 'you', tail: b.keyTail }
      : { set: false, source: null, tail: '' };
    const s = Object.assign({
      web: true, provider,
      model: (b && b.model) || null,
      baseUrl: (b && b.baseUrl) || null,
      /* the user's choice between their key and the NASJI model, and
         whether the key actually runs (saved and switched on) */
  ...
  const byokGet = async () => {
    ...
    if (res.status === 401) return byokStatus(null, { signedOut: true });
    if (!res.ok || !j) throw new Error((j && j.error) || ('HTTP ' + res.status));
    return byokStatus(j.byok);
  };
  const byokSet = async (patch) => {
    const p = patch && typeof patch === 'object' ? patch : {};
    const body = {};
    if (typeof p.AGENT_PROVIDER === 'string') body.provider = p.AGENT_PROVIDER;
    if (typeof p.AGENT_BASE_URL === 'string') body.baseUrl = p.AGENT_BASE_URL;
    /* '1' = run on the key, '0' = keep it and run on the NASJI model */
    if (typeof p.BYOK_ACTIVE === 'string') body.active = p.BYOK_ACTIVE === '1';
    for (const k of Object.keys(p)) {
      if (typeof p[k] !== 'string') continue;
      if (k === 'AGENT_MODEL' || /_MODEL$/.test(k)) body.model = p[k];
      const m = /^([A-Z]+)_API_KEY$/.exec(k);
      if (!m || m[1] === 'GEMINI') continue;
      body.key = p[k];
      if (m[1] !== 'AGENT') body.provider = m[1].toLowerCase();
    }
    const res = await postJson('/api/me/byok', body);
    const j = await res.json().catch(() => null);
    if (res.status === 401) throw new Error('Sign in first.');
    if (!res.ok || !j || !j.ok) throw new Error((j && j.error) || ('HTTP ' + res.status));
    return byokStatus(j.byok, { ok: true });
  };
